...
To start using the injector, simply create a new instance of the `Atreyu\Injector` ("the Injector")
class:
...
You may also specify injection definitions at call-time with `Atreyu\Injector::make`. Consider:
...
specified class, but only in the context of that particular call to `Atreyu\Injector::make`.
...
and should generally be avoided. `Atreyu\Injector` makes sharing class instances across contexts a
...
> By sharing an instance of a class, `Atreyu\Injector` will always use that instance when
...
`Atreyu\Injector::make` will have no effect. Once shared, an instance will always be returned for
...
`Atreyu\Injector::delegate`. Additionally you may specify the name of a delegate class that
...
`Atreyu\Injector` resolves dependencies in the following order:
...
3. If a call-time definition is passed to `Atreyu\Injector::make`, that definition will be used
...
> **IMPORTANT:** do not use Atreyu like a Service Locator!
...
see how we can use Atreyu to inject the same `PDO` instance across the entire scope of our application.
